...
// Deste bu şekilde. A = 1 veya 11 olablir. J, K ve Q 10 puana denk geliyor.
...
function checkBanned(msg){
    if(bannedUsers.includes(msg.author.id)){
        message.reply("üzgünüz, gazinomuzdan banlandınız.")
        return true;
    }
    else{
        return false;
...
// blackjackte eli hesaplamak için kullanılan kod 
...
bot.on('message', message => {
	...
	if(message.guild.id === '588434226488279052'){
		if(message.channel.id !== '607912313240027145'){
			return;
		}
	}
    ...
    /*
    if(message.author.id === "214508332919029760" || message.author.id === "116636410341097477"){
        console.log(message.author.username + ": " + new Date() + " " + args[0])
    }*/
    ...
    if (args[0] === "claim") {
        ...
        if(checkBanned(message) === true) return(message.reply("üzgünüz, gazinomuzdan banlandınız."));
        ...
        fs.writeFile("./money.json", JSON.stringify(money, null, 4), function(err) {
            if(err) {
              console.log(err);
            }
        });
    ...
    if(args[0] === "setmoney"){
        ...
        money[message.mentions.users.first().id].money = args[1];
        fs.writeFile("./money.json", JSON.stringify(money, null, 4), function(err) {
            if(err) {
              console.log(err);
            }
    ...
    // mafya mı devlet mi ne olduğu belli olmayanlar tarafından zorla yazdırılan kod bunu okuyorsanız lütfen yardım edin
    if (args[0] === "vergi") {
        if (message.author.id !== "298884333970784256") return;
    ...
    if (args[0] === "yardım") {
        const embed = new Discord.RichEmbed()
            ...
            .addField("!blackjack <bahis>", "Blackjack (21) oyununu başlatır. !kurallar blackjack yazarak nasıl oynandığını öğrenebilirsiniz.")
    ...
    if (args[0] === "kurallar") {
        if (args[1] === "zaroyunu") {
            ...
            const embed = new Discord.RichEmbed()
                ...
                .addField('3 x 💙', '250x')
                .addField('3x :heavy_dollar_sign:', '1000x')
                .setColor(0x97a7e6)
            message.channel.sendEmbed(embed).catch(err => console.log(err));;
            return;
        }
        ...
        else if(args[1] === "blackjack"){
            message.reply("Blackjackte amaç 21 sayısına mümkün olduğunca çok yaklaşabilmek ama 21'i geçmemektir. Başlangıçta hem oyuncu hem de kurupiye için 2 kart dağıtılır. Sizin kartlarınız açıkken kurupiyenin sadece 1 kartını görebilirsiniz. !hit yazarak yeni kart isteyebilir veya !stand yazarak turunuzu sonlandırabilirsiniz. Eğer 21'i geçerseniz oyunu otomatik olarak kaybedersiniz. Turunuz bittikten sonra kurupiye, elindeki kartların toplamı en az 17 olana kadar kart çeker. En son eliniz kurupiyenin elinden büyükse kazanır, değilse kaybedersiniz.")
        }
    }
    ...
    if (args[0] === "slot") {
        ...
        var symbolsBase = ["<:Kalpgul:603890144457785345>", "💙", ":spades:", ":six: :nine:", ":clown:", ":police_car:, :heavy_dollar_sign:"]
        var symbols = [":heavy_dollar_sign:", "<:Kalpgul:603890144457785345>", "💙", "💙", ":spades:", ":spades:", ":spades:", ":six: :nine:", ":six: :nine:", ":six: :nine:", ":six: :nine:", ":clown:", ":clown:", ":clown:", ":clown:", ":clown:", ":police_car:", ":police_car:", ":police_car:", ":police_car:", ":police_car:", ":police_car:"]
        var slotResults = []
        ...
        if (slotResults[0] === slotResults[1] && slotResults[0] === slotResults[2]) {
            ...
            else if(slotResults[0] === ":heavy_dollar_sign:"){
                message.reply("tebrikler, " + playerBet * 1000 + "$ kazandınız!")
                money[message.author.id].money += playerBet * 1000
            }
        }
    ...
    if(args[0] === "blackjack"){
        // Hesap var mı yok mu kontrol etsin
        if(!money[message.author.id]){
            message.reply("lütfen müessesemizin imkanlarından faydalanmak için !hesapaç komutu ile hesap açın.");
            return;
        }
        ...
        if(money[message.author.id].playingBlackjack === true){
            message.reply("lütfen önce başlattığınız oyunu bitirin.")
            return;
        }


        // !blackjack 1000 gibi bir bahis girilmesi gerekiyor, sayı girilmemişse hata veriyor.
        if(isNumeric(args[1]) === false){
            message.reply("lütfen geçerli bir sayı girin.");
            return;
        }
        ...
        if(parseInt(args[1]) > money[message.author.id].money){
            message.reply("yeterli bakiyeniz bulunmamakta.")
            return;
        }

        // Girilen bet hemen paradan çekiliyor ve kaydediliyor.
        var playerBet = parseInt(args[1]);
        money[message.author.id].blackjackBet = playerBet;
        money[message.author.id].money -= playerBet;
        money[message.author.id].playingBlackjack = true;
        fs.writeFile("./money.json", JSON.stringify(money, null, 4), function(err) {
                if(err) {
                  console.log(err);
                }
        ...
        // Eller dağıtılıyor
        for(var n = 0; n < 2; n++){
            money[message.author.id].blackjackHand.push(deck[Math.floor(randomNumber() * 11)])
            money[message.author.id].blackjackDealerHand.push(deck[Math.floor(randomNumber() * 11)])
        }

        // Eller hesaplanıyor
        var playerTotal = calculateHand(money[message.author.id].blackjackHand);
        var dealerTotal = calculateHand(money[message.author.id].blackjackDealerHand);

        message.reply("elinizdeki kartlar: " + money[message.author.id].blackjackHand[0] + ", " + money[message.author.id].blackjackHand[1] + " (Toplam: " + playerTotal + ")\nKurupiye kartı: " + money[message.author.id].blackjackDealerHand[0] + "\n!kurallar blackjack yazarak nasıl oynandığını öğrenebilirsiniz.");

        // direkt 21 gelirse kazanıyor
        if(playerTotal === 21){
            message.reply("blackjack! " + playerBet * 3 + "$ kazandınız!")
            money[message.author.id].money += playerBet * 3;
            money[message.author.id].playingBlackjack = false;
            money[message.author.id].blackjackDealerHand = [];
            money[message.author.id].blackjackHand = [];
            money[message.author.id].blackjackBet = 0;
            fs.writeFile("./money.json", JSON.stringify(money, null, 4), function(err) {
                if(err) {
                  console.log(err);
                }
        ...
        /*
        if(dealerTotal === 21){
            message.reply("kurupiyenin elinde blackjack var! Ne yazık ki kaybettiniz. (Diğer dealer kartı: " + money[message.author.id].blackjackDealerHand[1] + ")");
            money[message.author.id].playingBlackjack = false;
            money[message.author.id].blackjackDealerHand = [];
            money[message.author.id].blackjackHand = [];
            money[message.author.id].blackjackBet = 0;
    ...
    if(args[0] === "hit"){
        ...
        if(money[message.author.id].playingBlackjack === false){
            return;
        }

        var playerBet = money[message.author.id].blackjackBet;

        money[message.author.id].blackjackHand.push(deck[Math.floor(randomNumber() * 11)]);
        var playerTotal = calculateHand(money[message.author.id].blackjackHand);

        var handString = " "
        for(var n = 0; n < money[message.author.id].blackjackHand.length; n++){
            handString += money[message.author.id].blackjackHand[n] + ", "
        }

        message.reply("yeni eliniz: " + handString + " (Toplam: " + playerTotal + ")");
        ...
        if(playerTotal === 21){
            ...
            while(dealerTotal < 17){
                money[message.author.id].blackjackDealerHand.push(deck[Math.floor(randomNumber() * 11)]);
                dealerTotal = calculateHand(money[message.author.id].blackjackDealerHand);
            }

            var dealerHandString = " "
            for(var n = 0; n < money[message.author.id].blackjackDealerHand.length; n++){
                handString += money[message.author.id].blackjackDealerHand[n] + ", "
            }


            message.reply("kurupiye kart çekimini bitirdi ve eli şu şekilde: " + dealerHandString + "(Toplam: " + dealerTotal + ")");

            if(dealerTotal > 21){
                message.reply("kurupiyenin eli 21'i geçti. " + playerBet * 2 + "$ kazandınız.")
                money[message.author.id].money += playerBet * 2;
                money[message.author.id].playingBlackjack = false;
                money[message.author.id].blackjackDealerHand = [];
                money[message.author.id].blackjackHand = [];
                ...
                message.reply("sonuçlar eşit. Paranızı geri alabilirsiniz.")
                money[message.author.id].money += playerBet;
                money[message.author.id].playingBlackjack = false;
                money[message.author.id].blackjackDealerHand = [];
                money[message.author.id].blackjackHand = [];
    ...
    if(args[0] === "stand"){
        ...
        // Hesap var mı ve oyun başlatmış mı diye kontrol ediliyor
        if(!money[message.author.id]){
            return;
        }
        if(money[message.author.id].playingBlackjack === false){
